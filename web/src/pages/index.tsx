...
export default function Home({ poolCount, guessCount, userCount }: HomeProps) {
  ...
  async function createPool(e: FormEvent) {
    ...
    try {
      ...
      alert(
        "Bolão com sucesso! o código foi copiado para área de transferência!"
      );
    ...
    <div className="max-w-[1124px] h-screen mx-auto grid grid-cols-2 gap-28 items-center">
      <main>
        ...
        <div className="mt-10 pt-10 border-t border-gray-600 flex items-center justify-between">
          <CountItem count={poolCount} label="Bolões criados" />
          <div className="w-px h-14 bg-gray-600" />
          <CountItem count={guessCount} label="Palpites enviados" />
        </div>
      </main>
      <Image
        src={appPreviewImg}
...
function CountItem({ count, label }: any) {
  return (
